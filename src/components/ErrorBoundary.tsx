...
class ErrorBoundary extends Component<Props, State> {
  ...
  public render() {
    if (this.state.hasError) {
      return (
        <div className="flex h-screen w-full flex-col items-center justify-center bg-slate-50 dark:bg-slate-950 p-8 text-center font-sans">
          ...
          <h2 className="text-2xl font-black text-slate-900 dark:text-white mb-3 uppercase tracking-tighter italic text-center">
            Erro de Inicialização
          </h2>
          
          <p className="text-slate-500 dark:text-slate-400 text-sm max-w-md mb-10 leading-relaxed font-medium">
            O TaxiControl encontrou um problema crítico ao carregar os módulos. 
            Isto pode ser devido a uma falha de rede ou um erro de processamento interno.
          </p>
          ...
          <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-white/5 w-full max-w-lg mb-10 overflow-auto max-h-48 shadow-sm">
             <p className="text-[11px] font-mono text-slate-600 dark:text-slate-400 break-all text-left">
                ...
                {this.state.error?.stack && <span className="block mt-4 opacity-50">{this.state.error.stack.split('\n').slice(0, 3).join('\n')}</span>}
             </p>
          </div>
